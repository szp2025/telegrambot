...
class BotVirtualAssistant:
    def __init__(self, model_name: str = "Gemini AI Core"):
     ...
     def generate_response(self, userQuery: str, gameContext: dict = None) -> str:
        ...
        # 1. Анализ приветствий и общего общения
        if any(word in query_lower for word in ["привет", "здарова", "салют", "хай", "как дела", "что делаешь"]):
            return (
                f"🤖 **{self.model_name} на связи!**\n"
                f"Приветствую! Я ваш персональный виртуальный интеллект. Я готов помочь вам с анализом игровых стратегий, "
                f"расчетом окупаемости крипто-кранов, майнеров и авто-ферм.\n\n"
                f"Спросите меня, например: *«Какая стратегия лучше для прокачки?»* или опишите вашу текущую игровую ситуацию."
            )
            
        # 2. Анализ вопросов про стратегии и прокачку
        elif any(word in query_lower for word in ["стратег", "прокач", "лучше", "поднять", "зароб", "доход", "монет"]):
            return (
                f"🧠 **Глубокий ИИ-анализ стратегии:**\n\n"
                f"Я проанализировал ваш запрос (*«{userQuery}»*). В текущих реалиях игровых ботов и авто-ферм ключевым фактором успеха является баланс между пассивным доходом и регулярным сбором комбо.\n\n"
                f"📈 **Рекомендации по оптимизации:**\n"
                f"1. Распределяйте получаемые монеты пропорционально между активными майнерами.\n"
                f"2. Следите за таймерами тайм-менеджмента, чтобы не терять пассивную прибыль в период простоя.\n"
                f"3. Используйте защитные механизмы счета («Банковский Гамбит»), чтобы минимизировать риски при транзакциях.\n\n"
                f"Хотите разобрать конкретную игру из вашего списка? Напишите ее название!"
            )
            
        # 3. Умный контекстный ответ на абсолютно любой другой вопрос (разговорный режим)
        else:
            return (
                f"🧠 **{self.model_name} (Аналитический модуль):**\n"
                f"Я внимательно изучил ваше сообщение: *«{userQuery}»*.\n\n"
                f"Как искусственный интеллект, я считаю это направление перспективным. Для достижения максимального результата в боте рекомендую регулярно проверять разделы «Крипто-краны», «Авто-фермы» и следить за обновлениями комбо-карт.\n\n"
                f"Если у вас есть конкретная задача или вопрос по коду/стратегии — смело задавайте, я готов вести диалог!"
            )
