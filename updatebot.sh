...
# Список файлов для обновления (включая самообновление этого скрипта)
FILES_TO_UPDATE=(
...
for item in "${FILES_TO_UPDATE[@]}"; do
    # Разделяем строку на URL и целевой путь по двоеточию
    URL="${item%%:*}"
    TARGET_FILE="${item##*:}"
    ...
    if curl -s -L "$URL" -o "$TEMP_FILE"; then
        ...
        if [[ "$TARGET_FILE" == *.py ]]; then
            ...
            mv "$TEMP_FILE" "$TARGET_FILE"
            # Делаем скачанный .sh файл снова исполняемым
            chmod +x "$TARGET_FILE"
            echo "🚀 Файл $TARGET_FILE успешно обновлен!"
        fi
    else
